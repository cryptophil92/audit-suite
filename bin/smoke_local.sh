...
smoke_step "routes_json" bash -c 'bash bin/routes_json.sh | jq -e ".kind == \"audit-suite.routes\"" >/dev/null'
...
smoke_step "manifest_json" bash -c 'bash bin/manifest_json.sh validate tests/fixtures/findings/manifest-1.2.0.json | jq -e ".valid == true" >/dev/null'
